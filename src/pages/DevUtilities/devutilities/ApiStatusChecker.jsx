...
const NetworkRequestTester = () => {
  const { dark } = useTheme();
  const [input, setInput] = useState(""); //URL
  const [output, setOutput] = useState(""); //API RESULT
  ...
  const handleCheck = async () => {
    try {
      ...
      setOutput(
        JSON.stringify(
          {
            error: "Request failed (CORS or invalid URL)",
          },
          null,
          2,
        ),
  ...
  const handleSample = () => {
    setInput("https://api.github.com/users/octocat");
    setOutput("");
  };
  ...
  const buttons = [
    { label: "Check API", onClick: handleCheck },
  ...
  return (
    ...
    >
      <title>API Status Checker — DevTasks</title>
      <meta
        name="description"
        content="Check API endpoints, inspect HTTP status codes, response time and returned data directly in the browser."
      />
...
export default NetworkRequestTester;
